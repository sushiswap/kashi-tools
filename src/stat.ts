...
function getNetwork(name: string): Network {
    switch(name) {
        case 'ether': return networks.Ethereum
        case 'polygon': return networks.Polygon
        case 'kovan': return networks.Kovan
        default:
...
switch(process.argv[3]) {
    case 'insolvent': getAllKashiPairsBentoV1(network)
        break
    case 'borrowers': getBorrowers(network, process.argv[4] === undefined ? 50 : parseFloat(process.argv[4]))
        break
    default:
